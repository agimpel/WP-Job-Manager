 ...
 * @since 1.21.0
 */
class WP_Job_Manager_Widget_Industry_Jobs extends WP_Job_Manager_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = sprintf( __( 'Industry %s', 'wp-job-manager' ), $wp_post_types['job_listing']->labels->name );
		$this->widget_cssclass    = 'job_manager widget_featured_jobs';
		$this->widget_description = __( 'Display a list of Industry listings on your site.', 'wp-job-manager' );
		$this->widget_id          = 'widget_industry_jobs';
		$this->settings           = array(
			'title'   => array(
				'type'  => 'text',
				// translators: Placeholder %s is the plural label for the job listing post type.
				'std'   => sprintf( __( 'Industry %s', 'wp-job-manager' ), $wp_post_types['job_listing']->labels->name ),
	...
	public function widget( $args, $instance ) {
		...
		if ( $jobs->have_posts() ) : ?>
			...
			<a href="/job-region/industry/" class="button button--size-small" style="margin:auto;">View all offers in Industry</a>
			</div>
...
register_widget( 'WP_Job_Manager_Widget_Industry_Jobs' );
